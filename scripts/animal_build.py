#!/usr/bin/env python3
"""Assemble the Move Like the Animal Wake Up song short (runs in CI).
...
LOGO  = SPEC["logo"]
OUT  = SPEC["out"]
WORK = "_staging/animal_work"; os.makedirs(WORK, exist_ok=True)
...
print("Move Like the Animal: %d shots, video=%.2fs, audio=%.2fs, final=%.2fs -> %s"%(len(parts),vdur,adur,fd,OUT))
