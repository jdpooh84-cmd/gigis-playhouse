#!/usr/bin/env python3
"""Assemble the The Potty Dance song short (runs in CI).

Inputs:
  _staging/potty/shot_spec.json      20 equal shots (start/end/slot, overlays, logo flag)
  _staging/potty/video_results.json  {clip_id: video_url}
  _staging/audio/the-potty-dance.mp3   the song (muxed in, re-encoded to AAC for universal playback)
...
LOGO  = SPEC["logo"]
OUT  = SPEC["out"]
WORK = "_staging/potty_work"; os.makedirs(WORK, exist_ok=True)
...
print("The Potty Dance: %d shots, video=%.2fs, audio=%.2fs, final=%.2fs -> %s"%(len(parts),vdur,adur,fd,OUT))
