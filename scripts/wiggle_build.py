#!/usr/bin/env python3
"""Assemble the Wiggle Wake Up song short (runs in CI).

Inputs:
  _staging/wiggle/shot_spec.json      20 equal shots (start/end/slot, overlays, logo flag)
...
SPEC = json.load(open("_staging/wiggle/shot_spec.json"))
RES  = json.load(open("_staging/wiggle/video_results.json"))
AUDIO = SPEC["audio"]
LOGO  = SPEC["logo"]
OUT  = SPEC["out"]
WORK = "_staging/wiggle_work"; os.makedirs(WORK, exist_ok=True)
...
for i,c in enumerate(SPEC["clips"]):
    ...
    if c.get("logo"):
        ...
        run(["ffmpeg","-y","-v","error","-i",src,"-i",LOGO,"-filter_complex",fc,"-map","[o]",
...
print("Wiggle: %d shots, video=%.2fs, audio=%.2fs, final=%.2fs -> %s"%(len(parts),vdur,adur,fd,OUT))
