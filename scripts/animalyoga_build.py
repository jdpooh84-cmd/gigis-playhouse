#!/usr/bin/env python3
"""Assemble the Animal Yoga song short (runs in CI).

Inputs:
  _staging/animalyoga/shot_spec.json      20 equal shots (start/end/slot, overlays, logo flag)
  _staging/animalyoga/video_results.json  {clip_id: video_url}
  _staging/audio/animal-yoga.mp3   the song (muxed in, re-encoded to AAC for universal playback)
...
SPEC = json.load(open("_staging/animalyoga/shot_spec.json"))
RES  = json.load(open("_staging/animalyoga/video_results.json"))
AUDIO = SPEC["audio"]
LOGO  = SPEC["logo"]
OUT  = SPEC["out"]
WORK = "_staging/animalyoga_work"; os.makedirs(WORK, exist_ok=True)
os.makedirs(os.path.dirname(OUT), exist_ok=True)
TAIL_SLOW_REGION = 5.0
...
print("Animal Yoga: %d shots, video=%.2fs, audio=%.2fs, final=%.2fs -> %s"%(len(parts),vdur,adur,fd,OUT))
