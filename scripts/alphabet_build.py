#!/usr/bin/env python3
"""Assemble the Alphabet Adventure Train Wake Up song short (runs in CI).

Inputs:
  _staging/alphabet/shot_spec.json      20 equal shots (start/end/slot, overlays, logo flag)
  _staging/alphabet/video_results.json  {clip_id: video_url}
  _staging/audio/alphabet-adventure-train.mp3   the song (muxed in, re-encoded to AAC for universal playback)
...
SPEC = json.load(open("_staging/alphabet/shot_spec.json"))
RES  = json.load(open("_staging/alphabet/video_results.json"))
AUDIO = SPEC["audio"]
LOGO  = SPEC["logo"]
OUT  = SPEC["out"]
WORK = "_staging/alphabet_work"; os.makedirs(WORK, exist_ok=True)
...
print("Alphabet Adventure Train: %d shots, video=%.2fs, audio=%.2fs, final=%.2fs -> %s"%(len(parts),vdur,adur,fd,OUT))
...
print("Alphabet Adventure Train OK.")
