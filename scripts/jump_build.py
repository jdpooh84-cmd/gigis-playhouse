#!/usr/bin/env python3
"""Assemble the Jump With Me song short (runs in CI).

Inputs:
  _staging/jump/shot_spec.json      20 equal shots (start/end/slot, overlays, logo flag)
  _staging/jump/video_results.json  {clip_id: video_url}
  _staging/audio/jump-with-me.mp3   the song (muxed in, re-encoded to AAC for universal playback)
...
SPEC = json.load(open("_staging/jump/shot_spec.json"))
RES  = json.load(open("_staging/jump/video_results.json"))
CAPS_PATH = "_staging/jump/captions.json"
CAPS = json.load(open(CAPS_PATH)) if os.path.exists(CAPS_PATH) else None
AUDIO = SPEC["audio"]
LOGO  = SPEC["logo"]
OUT  = SPEC["out"]
WORK = "_staging/jump_work"; os.makedirs(WORK, exist_ok=True)
...
def cap_draw(text, t0, t1):
    # ABSOLUTE-timeline caption (from the real transcript), lower-third, teal border, quick fade.
    en = "between(t\\,%.3f\\,%.3f)" % (t0, t1)
...
for i,c in enumerate(SPEC["clips"]):
    ...
    # transcript-derived captions burned globally after concat; else fall back to spec overlays
    if CAPS is None:
...
print("Jump With Me: %d shots, video=%.2fs, audio=%.2fs, final=%.2fs -> %s"%(len(parts),vdur,adur,fd,OUT))
