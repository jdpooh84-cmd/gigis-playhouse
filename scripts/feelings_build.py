#!/usr/bin/env python3
"""Assemble the How Do You Feel song short (runs in CI).

Inputs:
  _staging/feelings/shot_spec.json      20 equal shots (start/end/slot, overlays, logo flag)
  _staging/feelings/video_results.json  {clip_id: video_url}
  _staging/audio/how-do-you-feel.mp3   the song (muxed in, re-encoded to AAC for universal playback)
...
SPEC = json.load(open("_staging/feelings/shot_spec.json"))
RES  = json.load(open("_staging/feelings/video_results.json"))
CAPS_PATH = "_staging/feelings/captions.json"
CAPS = json.load(open(CAPS_PATH)) if os.path.exists(CAPS_PATH) else None
AUDIO = SPEC["audio"]
LOGO  = SPEC["logo"]
OUT  = SPEC["out"]
WORK = "_staging/feelings_work"; os.makedirs(WORK, exist_ok=True)
...
for i,c in enumerate(SPEC["clips"]):
    ...
    # When a transcript-derived caption track exists, burn NO per-shot text here — captions are
    # burned globally after concat, timed to the real audio. Otherwise fall back to spec overlays.
    if CAPS is None:
...
print("How Do You Feel: %d shots, video=%.2fs, audio=%.2fs, final=%.2fs -> %s"%(len(parts),vdur,adur,fd,OUT))
