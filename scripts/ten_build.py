#!/usr/bin/env python3
"""Assemble the Ten Little Fingers Folding Down Wake Up song short (runs in CI).

Inputs:
  _staging/ten/shot_spec.json      20 equal shots (start/end/slot, overlays, logo flag)
  _staging/ten/video_results.json  {clip_id: video_url}
  _staging/audio/ten-little-fingers.mp3   the song (muxed in, re-encoded to AAC for universal playback)
...
SPEC = json.load(open("_staging/ten/shot_spec.json"))
RES  = json.load(open("_staging/ten/video_results.json"))
AUDIO = SPEC["audio"]
LOGO  = SPEC["logo"]
OUT  = SPEC["out"]
WORK = "_staging/ten_work"; os.makedirs(WORK, exist_ok=True)
...
def ov_draw(kind, text, t0, t1):
    # lullaby overlays: pale, gentle, with a soft alpha fade in/out (~0.7s) — never harsh
    en = "between(t\\,%.3f\\,%.3f)" % (t0, t1)
    al = "alpha='clip(min((t-%.3f)/0.7\\,(%.3f-t)/0.7)\\,0\\,1)'" % (t0, t1)
    ...
    if kind == "end":
        return (",drawtext=%s:y=(h-th)/2:fontsize=%d:fontcolor=0xFFF3D6:borderw=2:"
                "bordercolor=0x2A1F4A:box=1:boxcolor=0x140F2E@0.32:boxborderw=34:"
                "shadowcolor=black@0.4:shadowx=2:shadowy=2" % (common, fit_fs(text,86)))
    # soft — pale lower-third, gentle box for legibility on the dark night palette
    return (",drawtext=%s:y=h-160:fontsize=%d:fontcolor=0xF3F0FF:borderw=2:"
            "bordercolor=0x241A3D:box=1:boxcolor=0x140F2E@0.28:boxborderw=26:"
            "shadowcolor=black@0.4:shadowx=2:shadowy=2" % (common, fit_fs(text,66)))
...
print("Ten Little Fingers Folding Down: %d shots, video=%.2fs, audio=%.2fs, final=%.2fs -> %s"%(len(parts),vdur,adur,fd,OUT))
...
print("Ten Little Fingers Folding Down OK.")
