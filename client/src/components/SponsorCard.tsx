...
import { useAdminStore, type Sponsor } from "@/lib/admin-store";
...
export default function SponsorCard({ position }: SponsorCardProps) {
  ...
  const incrementImpressions = useAdminStore((s) => s.incrementImpressions);

  // Only show approved sponsors that haven't hit their cap
  const eligibleSponsors = useMemo(
    () =>
      sponsors.filter(
        (s) =>
          s.status === "approved" &&
          s.impressions_this_month < s.impression_cap_monthly
      ),
    [sponsors]
  );

  // Deterministic sponsor selection based on position
  ...
  const handleClick = useCallback(() => {
    if (sponsor) {
      incrementImpressions(sponsor.id);
      // Open CTA URL in new tab (parent-facing)
      if (sponsor.cta_url) {
        window.open(sponsor.cta_url, "_blank", "noopener,noreferrer");
      }
    }
  }, [sponsor, incrementImpressions]);
