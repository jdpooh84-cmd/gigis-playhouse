...
- **VISUAL-STYLE LOCK GATE (HARD, creator-locked 2026-07-14):** the show is **theatrical-feature-quality polished stylized 3D CGI.** Any shot reading **2D / flat / outlined / storybook-illustration / web-broadcast-educational** = **defective → re-render in the locked 3D style.** Every image prompt carries the STYLE BLOCK and none of the banned words (storybook, cartoon, illustration, 2D, flat, vector, Pixar). No change to rendering style / dimensionality / model fidelity without explicit creator approval. Full spec: `30_visual_style_lock_and_ep01_audit.md`. (Triggered by EP01 SEC-03 pilot group stills flattening to 2D.)
